...
def compress(action: Action | dict[str, Any]) -> bytes:
    ...
    Example:
        from hull.operator.action import action
        from hull.ledger.solana.action import compress
...
def hash(action: Action | dict[str, Any]) -> str:
    ...
    Example:
        from hull.operator.action import action
        from hull.ledger.solana.action import hash
...
def merkle_root(actions: list[Action | dict[str, Any]]) -> str:
    ...
    Example:
        from hull.operator.action import chain, action
        from hull.ledger.solana.action import merkle_root
...
def proof(
    ...
    Example:
        from hull.operator.action import action
        from hull.ledger.solana.action import proof
...
def size(actions: list[dict[str, Any]]) -> int:
    ...
    Example:
        from hull.operator.action import chain, action
        from hull.ledger.solana.action import size
...
def verify(action: Action | dict[str, Any], expected_hash: str) -> bool:
    ...
    Example:
        from hull.ledger.solana.action import hash, verify
...
def batch_compress(actions: list[Action | dict[str, Any]]) -> bytes:
    ...
    Example:
        from hull.operator.action import chain, action
        from hull.ledger.solana.action import batch_compress
