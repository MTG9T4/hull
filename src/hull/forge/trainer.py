"""
Training logic for HULL algorithms
"""
...
from ..engines.base import Algorithm
...
class Trainer:
    """Behavioral trainer that forges pilot policies from experience."""
    ...
    async def _validate(self, dataset: Dataset, batch_size: int) -> dict[str, Any]:
        ...
        for batch in dataset.iterate_batches(batch_size, shuffle=False):
            ...
            for sample in batch_data:
                ...
                if self._actions_match(predicted, sample["maneuver"]):
                    correct += 1
    ...
    def evaluate(
        ...
        for sample in test_dataset.samples:
            if sample.get("maneuver") and hasattr(sample["maneuver"], "type"):
                action_type = sample["maneuver"].type
