"""
Dataset management for training HULL algorithms
"""
...
class Dataset:
    ...
    def _step_to_sample(
        ...
        """
        if not step.get("maneuver") or step.get("error"):
            return None
        ...
        sample = {
            ...
            "maneuver": self._reconstruct_action(step.get("maneuver")),
    ...
    def _reconstruct_action(self, action_data: dict | None) -> Maneuver | None:
        """Reconstruct Maneuver object from serialized data"""
        if not action_data:
            return None
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        for sample in self.samples:
            if sample and sample.get("maneuver"):
                maneuver = sample["maneuver"]
                if maneuver is not None and hasattr(maneuver, "type"):
                    action_type = maneuver.type
