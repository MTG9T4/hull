...
def test_action_spread_params():
    ...
    a = maneuver("move", joint="arm", position=[1, 2, 3], speed=0.5)
    assert a.type == "move"
    assert a.joint == "arm"
...
def test_chain():
    ...
    a1 = maneuver("click", x=100, y=200)
    a2 = maneuver("type", text="hello")
    a3 = maneuver("wait", duration=1)
...
def test_batch():
    ...
    a1 = maneuver("fetch", url="api/1")
    a2 = maneuver("fetch", url="api/2")
...
def test_sequence():
    """Test creating maneuver sequences"""
    a1 = maneuver("click", x=100)
    a2 = maneuver("type", text="test")
...
def test_capture():
    """Test capture maneuver"""
    a = capture()
    assert a.type == "capture"


def test_record():
    """Test record maneuver"""
...
def test_parallel():
    """Test parallel maneuver execution"""
    a1 = maneuver("task1")
    a2 = maneuver("task2")
...
def test_retry():
    """Test retry maneuver with backoff"""
    a = maneuver("api_call", endpoint="/data")

    result = retry(a, attempts=5, delay=2.0)
    assert result.type == "retry"
    assert result.maneuver.type == "api_call"
    assert result.attempts == 5
    assert result.delay == 2.0


def test_throttle():
    """Test throttle maneuver"""
    a = maneuver("scroll")

    result = throttle(a, rate=0.1)
    assert result.type == "throttle"
    assert result.maneuver.type == "scroll"
    assert result.rate == 0.1


def test_debounce():
    """Test debounce maneuver"""
    a = maneuver("search", query="test")

    result = debounce(a, delay=0.3)
    assert result.type == "debounce"
    assert result.maneuver.query == "test"
    assert result.delay == 0.3


def test_action_type_annotation():
    """Test that Maneuver type works correctly"""

    def execute(a: Maneuver) -> str:
        return a.type

    a = maneuver("test", value=42)
    assert execute(a) == "test"


def test_nested_actions():
    """Test nesting actions within actions"""
    inner = chain(maneuver("step1"), maneuver("step2"))

    outer = sequence(maneuver("setup"), batch(*inner), maneuver("cleanup"))
...
def test_action_with_complex_params():
    """Test maneuver with nested complex params"""
    a = maneuver(
        "complex",
