"""
Flow matching utilities for continuous maneuver generation
"""
...
def compute_flow_velocity(
    ...
    Args:
        current: Current state/maneuver
        target: Target state/maneuver
...
def create_flow_model(
    ...
    class FlowModel(nn.Module):
        ...
        def forward(
            ...
            # Prepare inputs
            inputs = [maneuver, features]

            if self.time_emb is not None:
                t_emb = self.time_emb(t)
                # Expand time embedding to match maneuver sequence length
                if maneuver.dim() == 3 and t_emb.dim() == 2:
                    t_emb = t_emb.unsqueeze(1).expand(-1, maneuver.size(1), -1)
                inputs.append(t_emb)
