...
combining vision-language understanding with flow-based maneuver generation.
"""
...
class ActionEncoder(nn.Module):
    """
    Encoder for maneuver sequences with time conditioning
    """
...
class PI0(Algorithm):
    ...
    - Flow matching for maneuver generation
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        Args:
            config: Configuration including:
                - action_dim: Maneuver space dimension
    ...
    async def predict(
        ...
        """
        Predict maneuver using flow matching
        ...
        with torch.no_grad():
            ...
            # Initialize maneuver with noise
            batch_size = 1
    ...
    async def train(
        ...
        for sample in data:
            ...
            # Get target actions
            target_actions = self._action_to_tensor(maneuver)
            ...
            self.stored_patterns.append(
                {
                    ...
                    "maneuver": target_actions.detach().cpu(),
                }
            )
        ...
        if validation_data:
            ...
            for val_sample in validation_data:
                with torch.no_grad():
                    ...
                    maneuver = val_sample.get("maneuver")
    ...
    def _action_to_tensor(self, maneuver: Maneuver) -> torch.Tensor:
        """
        Convert maneuver to tensor
        ...
        Returns:
            Maneuver tensor
        """
        if hasattr(maneuver, "params") and maneuver.params:
            values = maneuver.params.get("values", [])
        else:
            values = getattr(maneuver, "values", [])
