...
class Imitation(Algorithm):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        self.policy: dict[str, Maneuver] = {}

    async def predict(
        self, state: State, context: dict[str, Any] | None = None
    ) -> Maneuver:
        """
        Predict maneuver based on learned demonstrations
        ...
        if best_match:
            return best_match["maneuver"]

        # Default maneuver if no match found
        return maneuver("wait")
    ...
    async def train(
        ...
        Args:
            data: List of (state, maneuver) pairs
        ...
        for demo in data:
            ...
            maneuver = demo.get("maneuver")

            if state and maneuver:
                state_key = self._state_to_key(state)
                self.policy[state_key] = maneuver
    ...
    def save(self, path: str):
        ...
        for k, v in self.policy.items():
            if hasattr(v, "__dict__"):
                ...
                # Maneuver is a dict-like object
                try:
    ...
    def load(self, path: str):
        ...
        for k, v in data.get("policy", {}).items():
            ...
            self.policy[k] = maneuver(action_type, **params)
