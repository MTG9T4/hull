...
async def main():
    # Create pilot with Brewie bridge
    op = Pilot({"brewie": BrewieRobot(host="192.168.1.100", port=9090)})
...
1. Create a new file in `src/hull/bridge/robot/` (e.g., `yourrobot.py`)
...
The base `Robot` class provides default implementations for `get_state()` and `execute_maneuver()` that work with joint positions.
